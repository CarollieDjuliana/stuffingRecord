...
import { getStorage, ref, uploadBytes, getDownloadURL, listAll } from "https://www.gstatic.com/firebasejs/9.10.0/firebase-storage.js";
...
document.querySelectorAll('[id^="openCameraButton"]').forEach((button, index) => {
    button.addEventListener('click', () => {
      ...
      // Cek apakah gambar sudah diupload sebelumnya
      if (imageUploaded[index]) {
        // Tampilkan konfirmasi untuk mengganti gambar
        const confirmChange = window.confirm('Apakah Anda ingin mengganti foto sebelumnya?');
        if (!confirmChange) {
            event.preventDefault(); // Batalkan akses kamera jika pengguna memilih untuk tidak mengganti foto
            return;
        }
    }
...
//untuk close activity
function countUploadedPhotos(shipper, no_booking, container) {
    const imagesFolder = 'images/' + shipper + '/' + no_booking + '/' + container;

    // Ambil daftar file dari folder di Firebase Storage
    return listAll(ref(storage, imagesFolder))
        .then((result) => {
...
function updateContainerStatus(shipper, no_booking, container) {
    // Hitung jumlah foto yang sudah diupload
    countUploadedPhotos(shipper, no_booking, container)
        .then((uploadedPhotosCount) => {
            // Memeriksa apakah sudah ada 6 foto yang diupload
            if (uploadedPhotosCount === 6) {
                // Update status container menjadi "COMPLETE"
                const containerPath = `activity/${shipper}/${no_booking}/container_data/${container}`;
                update(ref(db, containerPath), { status: "COMPLETE" })
                    .then(() => {
                        console.log(`Status container ${container} updated to COMPLETE`);
                    })
                    .catch((error) => {
                        console.error(`Error updating status for container ${container}: `, error);
                    });
            } else {
                console.log(`Total uploaded photos for container ${container}: ${uploadedPhotosCount}`);
            }
        })
        .catch((error) => {
            console.error('Error counting uploaded photos:', error);
        });
}



        const imageUploaded = [false, false, false, false, false, false];
...
document.querySelectorAll('[id^="uploadButton"]').forEach((uploadButton, index) => {
    uploadButton.addEventListener('click', () => {
        ...
        if (fileToUpload) {
            ...
            updateContainerStatus(shipper, no_booking, container);
           ...
           // Setelah mengambil gambar, simpan statusnya ke dalam localStorage
            ...
            uploadTask.then((snapshot) => {
             ...
             // Setelah berhasil mengunggah gambar
uploadedImage.setAttribute('data-uploaded', 'true');
      ...
      let successfullyFetchedPhotos = 0;
        ...
        function enableCloseActivityButton() {
            ...
            const closeButton = document.getElementById('closeActivityButton');
          
            // Aktifkan tombol "Close Activity"
            closeButton.disabled = false;
          
            // Tambahkan event listener untuk menangani penutupan aktivitas
            closeButton.addEventListener('click', closeActivityButton);
          }
