...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Meng-handle klik tombol "Close This Activity"
document.getElementById("closeActivityButton").addEventListener("click", function () {
  // Memanggil fungsi untuk menghitung jumlah foto yang sudah diupload
  countUploadedPhotos()
      .then((uploadedPhotosCount) => {
          // Memeriksa apakah sudah ada 6 foto yang diupload
          if (uploadedPhotosCount === 6) {
              // Menampilkan modal konfirmasi jika sudah ada 6 foto
              const confirmationModal = new bootstrap.Modal(document.getElementById('confirmationModal'));
              confirmationModal.show();
          } else {
              // Menampilkan alert jika belum ada 6 foto
              alert('Lengkapi semua foto terlebih dahulu!');
          }
      })
      .catch((error) => {
