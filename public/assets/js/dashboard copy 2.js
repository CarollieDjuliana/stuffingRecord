          ...
          for (const noBookingKey in shipper) {
            if (shipper.hasOwnProperty(noBookingKey)) {
              ...
              if (typeof noBooking === 'object' && noBooking !== null) {
                const no_booking = noBooking.no_booking;
                const shipper_ = noBooking.shipper;
                const date = new Date(noBooking.date);
                const status = noBooking.status;
                const customer = noBooking.customer;
                var noBookingWithoutSymbol = no_booking;
                // Periksa apakah nomor booking mengandung simbol '/'
                if (no_booking.includes('/')) {
                  // Jika iya, ambil bagian sebelum simbol '/'
                  const parts = no_booking.split('/');
